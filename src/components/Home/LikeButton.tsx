...
import { dislikePicture, dislikePost, likePicture, likePost } from "@/feature/likeSlice";
import { savePost } from "@/feature/saveSlice";
...
export default function LikeButton(props: LikeButtonProps) {
    ...
    const like = () => {
        if (!props.liked) {
            if(props.type === "post") {
                dispatch(likePost(props.post as Post));
            }
            else {
                dispatch(likePicture(props.post as Picture));
            }
        }
        else {
            if(props.type === "post") {
                dispatch(dislikePost(props.post.id));
            }
            else {
                dispatch(dislikePicture(props.post.id));
            }
        }
    ...
    if (props.liked) {
        if (document.getElementById(JSON.stringify(props.post.id)) as HTMLInputElement) {
            (document.getElementById(JSON.stringify(props.post.id)) as HTMLInputElement).checked = true;
        }
    }

    return (
        <div className="con-like">
            <input className="like" id={JSON.stringify(props.post.id)} type="checkbox" onChange={like} checked={props.liked} title="like"/>
